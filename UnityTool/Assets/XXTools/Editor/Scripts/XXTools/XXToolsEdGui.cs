...
{
    public class XXToolsEdGui : MonoBehaviour
    {

        // ================================================================================================================
        #region gui style/skin

        public const string EditorResourcePath = "Assets/XXTools/Editor/Res/";
            ...
            EditorGUILayout.BeginVertical(MenuBoxStyle, options);
            {
                ...
                for (int i = 0; i < menuItems.Length; i++)
                {
                    ...
                    if (menuItems[i][0] == '-')
                    {
                        ...
                        {
                            GUILayout.Label(menuItems[i].Substring(1), XXToolsEdGui.MenuHeadStyle);
                        }
                        else
                        {
                    ...
                    if (GUILayout.Toggle((i == selected), menuItems[i], XXToolsEdGui.MenuItemStyle)) selected = i;
                }
        ...
        public static GameObject GameObjectFieldWithPreview(string label, GameObject obj, bool allowSceneObjects, GUIStyle style = null, int previewWidth = 64, int previewHeight = 64)
        {
            ...
            Rect r = EditorGUILayout.BeginVertical(style, GUILayout.Width(fieldW), GUILayout.Height(fieldH), GUILayout.ExpandWidth(false), GUILayout.ExpandHeight(false));
            {
                ...
                if (obj == null)
                {
                    GUI.DrawTexture(r2, XXToolsEdGui.Texture_FlatBox);
        ...
        public static string PathField(string path, string label, string dialogTitle)
        {
            EditorGUILayout.BeginHorizontal();
            {
                EditorGUILayout.LabelField(label, path, XXToolsEdGui.TextFieldStyle);
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (GUILayout.Button("Browse", GUILayout.Width(80)))
                {
                    ...
                    {   // make into relative path again
                        path = XXToolsEdUtil.ProjectRelativePath(newPath);
                    }
                }
            }
            EditorGUILayout.EndHorizontal();
        ...
        {
            int fieldToDel = -1;
            EditorGUILayout.BeginVertical(XXToolsEdGui.BoxStyle, GUILayout.Width(width), GUILayout.ExpandWidth(false));
            {
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    {
                        GUILayout.Label(label, XXToolsEdGui.Head3Style);
                        GUILayout.FlexibleSpace();
                    }

                    if (XXToolsEdGui.IconButton("Add", XXToolsEdGui.Icon_Plus, EditorStyles.miniButton, GUILayout.Width(80)))
                    {
                ...
                scroll = XXToolsEdGui.BeginScrollView(scroll, GUILayout.Width(width - 20), GUILayout.Height(height));
        ...
        public static bool FoldableLabel(bool foldout, string label, int width)
        {
            EditorGUILayout.BeginHorizontal();
            {
                ...
                if (foldout) XXToolsEdGui.LookLikeControls(0, 8);
                foldout = EditorGUILayout.Foldout(foldout, (foldout ? "" : label));
                XXToolsEdGui.LookLikeControls();
        ...
        public static Vector2 BeginScrollView(Vector2 scroll, bool hideAllBars, GUIStyle backgroundStyle, params GUILayoutOption[] options)
        {
            ...
            return EditorGUILayout.BeginScrollView(scroll, false, false, GUIStyle.none, GUIStyle.none, XXToolsEdGui.Skin.scrollView, options);
        }
        ...
        {
            return EditorGUILayout.BeginScrollView(scroll, false, true, GUIStyle.none, XXToolsEdGui.Skin.verticalScrollbar, (backgroundStyle == null ? XXToolsEdGui.Skin.scrollView : backgroundStyle), options);
        }
        ...
        public static void DrawHorizontalLine(float thickness, Color color, float paddingTop = 0f, float paddingBottom = 0f, float width = 0f)
        {
            ...
            r.y += paddingTop;
            r.height = thickness;
            GUI.Box(r, "", XXToolsEdGui.DividerStyle);
